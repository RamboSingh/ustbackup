...
public class ProfileAction {
	...
	public void profile_Launch() throws Exception {
		...
		try {
			boolean element1 = mblock.ElementExists(ObjDashboard.lnk_profile, 5000);
			if (element1) {
	...
	public void NegMailingAddress() throws Exception {
		try {
			...
			if (element1) {
				...
				if (maxChar != enteredVal) {
					mblock.ValidateTest(true, true, "Street Address accepts only 100 characters");
				}
				...
				if (element2) {
					...
					if (maxCharApt != enteredValApt) {
						mblock.ValidateTest(true, true, "Apt/Suite accepts only 25 characters");
					}
				...
				if (element3) {
					...
					if (maxCharCity != enteredValCity) {
						mblock.ValidateTest(true, true, "City accepts only 50 characters");
					}
	...
	public void NegPhone() throws Exception {
		try {
			...
			if (element1) {
				...
				if (maxChar != enteredVal) {
					mblock.ValidateTest(true, true, "Phone number accepts only 50 numbers");
				} else if (mblock.ElementExists(objProfile.Error_HomePh) == true) {

					mblock.ValidateTest(false, true, "Entered number is not of 10 digits");
	...
	// Subscribe
	/*
	 * public void chkBox_Subscribe() throws Exception { try { boolean element1
	 * = mblock.ElementExists(objProfile.CheckBox_receiveAlerts); if (element1)
	 * { mblock.ValidateTest(true, true, "Check Box is displayed");
	 * mblock.Log("Clicking on Check Box"); boolean chckbox =
	 * mblock.Element(objProfile.CheckBox_receiveAlerts).isSelected(); if
	 * ((chckbox == false) && (ExcelAction.crtnls[56].equalsIgnoreCase("YES")))
	 * { mblock.Element(objProfile.CheckBox_receiveAlerts).click();
	 * 
	 * } else { mblock.Element(objProfile.CheckBox_receiveAlerts).click(); }
	 * 
	 * } else {
	 * 
	...
	public void Btn_submit() throws Exception {
		try {
			...
			if (element1) {
				...
				mblock.Element(objProfile.Btn_Submit).ufxScrollElementToView();
	...
	public void Message_Success() throws Exception {
		try {
			...
			if (element1 == true) {
				mblock.Element(objProfile.Sucess_Status).ufxScrollElementToView();
				mblock.ElementExists(ObjDashboard.answr_col1,3000);  //this is line for just waiting 
