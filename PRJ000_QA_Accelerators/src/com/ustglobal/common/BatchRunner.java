...
public class BatchRunner  {	
	...
	public static void Launch(){ //This is a static method. A java program can call this as a utility method.	
		...
		try {//Batch runner could be opened as multiple instances by users, and from different places. Hence non static			
	...
	public BatchRunner(ArrayList<String> listExclusions)  {	
		...
		scrollPane = new JScrollPane(txtConfigData,ScrollPaneConstants.VERTICAL_SCROLLBAR_NEVER, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
		...
		lstboxApplyConfigScope.setModel(new DefaultComboBoxModel(new String[] {"All rows", "Selected", "Highlighted"}));
		        ...
		        if(nCol == nColExecutionStatus){
		        	...
		        	if(strReportFilePath!=null){
		        		try{
				        	...
				        	JOptionPane.showMessageDialog(frame, "The report file is currently not available, or cannot be opened", "File not found", JOptionPane.PLAIN_MESSAGE);
						}
		        	}
		        }
		    }
	...
	private void RefreshAndReloadGUI(){ //This will be called initially, or on pressing the Refresh button
		...
		files.forEach((file) -> {	
			int nCount=0;
			String strPackageAndClassName = file.toString().replace(strWorkingDirectory,"").replace(".class", "").replace("\\",".");
			if(!listExclusions.contains(strPackageAndClassName)){//BatchRunner doesn't select class files from standard libraries
				try {
					if(Class.forName(strPackageAndClassName).getMethod("main", String[].class)!=null){
						...
						if(nIndexEndPackageName!=-1){ //TODO: test the default package cases. Group them as --default package-- item
							String sPackageName = strPackageAndClassName.substring(0, nIndexEndPackageName);
							lstboxPackageFilter.addItem(sPackageName); //TODO: implement a JSON datastructure to filter as well as serialize user cache file
						}				   
					}
				}
				catch (Exception e1) {
	...
	private void ExecuteBatchRuns(boolean boolRunNotJustHighlighted){ //boolNotJustHighlighted=true implies run all user selected "Y" ones
		...
		ExecutorService objMonitor = Executors.newFixedThreadPool(nCountParallelTests);	//count of parallel monitors at a time	
		ExecutorService objExecutor = Executors.newFixedThreadPool(nCountParallelTests); //count of parallel threads at a time		
...
//			final String strTestCaseConfig = model.getValueAt(nRow, nColTestConfig).toString().trim();
//			final Object[] args = new Object[]{new String[]{strTestCaseConfig}};
			String strTestCaseConfig = model.getValueAt(nRow, nColTestConfig).toString().trim();
			//If individual testcase config is blank, then take it from general config
			...
			boolean boolExecute=boolRunNotJustHighlighted?(strSelection.equals("YES") || strSelection.equals("Y")):table.isRowSelected(nRowCurrent);			
			if(boolExecute){						
					...
					public void run() {			        	
								...
								while(true){					
									try {																		
										if(!objTestClassThread.isAlive()){
											strReportPath = mapThreadsMonitor.get("THREAD_ID" + nThreadIDChild + "_REPORT_PATH");
											strExecStatus = mapThreadsMonitor.get("THREAD_ID" + nThreadIDChild + "_TEST_STATUS");
											if(strExecStatus!=null){
												for(int nAttempt=0;nAttempt<=120;nAttempt++){ //Sometimes there is a time lag
													strExecStatus = mapThreadsMonitor.get("THREAD_ID" + nThreadIDChild + "_TEST_STATUS");
													if(strExecStatus.equals("")){
														model.setValueAt("Finalizing(" + nAttempt + ")..", nRowCurrent, nColExecutionStatus);
														Thread.sleep(500);
													}
													else{
														break;
												...
												model.setValueAt(strExecStatus.equals("")?"Completed":strExecStatus, nRowCurrent, nColExecutionStatus);
												strReportPath = mapThreadsMonitor.get("THREAD_ID" + nThreadIDChild + "_REPORT_PATH");
												mapRowsToReportFile.put("R"+nRowCurrent+"C"+nColExecutionStatus, strExecStatus.equals("")?null:strReportPath);
											}
											else{ //reporter or client was not initialized or used, however the test case execution completed
			    		...
			    		try {
			        	   ...
			        	   method.invoke(strPackageAndClassName, args);
			    			...
			    			e.printStackTrace(); //comment this to avoid unnecessary red remarks in the console
			    			System.out.println("An exception was handled in BatchRunner (the user killed a test case thread externally. This is a conceived error, and not critical)");
			    		}			    		
			    		objTestClassThread.stop(); //important. SystemTrayMonitor gets the interrupt abruptly			    		
		...
		objSupervisor.execute( new Runnable() { 
			...
			public void run() {	        	
        		try {
        			...
        			//System.out.println("skip count"+ nSkipCount);
        			//System.out.println("fail"+nFailCount);
        			try {
						CreatePdfSuammary();
					} catch (Exception e) {
...
//((DefaultTableCellRenderer)table.getCellRenderer(nRowCurrent, nColExecutionStatus)).setBackground(Color.YELLOW);
//.getTableCellRendererComponent(table, "Running...", false, false, nRowCurrent, nColExecutionStatus);
//DefaultTableCellRenderer objCellRenderer = new DefaultTableCellRenderer();
//objCellRenderer.setBackground(Color.YELLOW);
//DefaultTableCellRenderer objCellRenderer = new DefaultTableCellRenderer();
//objCellRenderer.getTableCellRendererComponent(table, "Running...", false, false, nRowCurrent, nColExecutionStatus);

/*public static void ReportExecutionStatus(long nThreadIDReportCreation, String strExecStatus, String strPathPDFReport){
	System.out.println("THREAD_ID_" + nThreadIDReportCreation +"_BATCH_TABLE_ROW");
	if(!BatchRunner.mapThreadToTableRow.containsKey("THREAD_ID_" + nThreadIDReportCreation +"_BATCH_TABLE_ROW")){
		System.out.println("The status reporting call to BatchRunner.ReportExecutionStatus( ) is ignored");
		return;
	}
	int nRowCurrent = BatchRunner.mapThreadToTableRow.get("THREAD_ID_" + nThreadIDReportCreation +"_BATCH_TABLE_ROW");
	DefaultTableModel model = (DefaultTableModel) BatchRunner.mapThreadToTableModel.get("THREAD_ID_" + nThreadIDReportCreation +"_BATCH_TABLE_MODEL");
	JButton btnResult = new JButton(strExecStatus);
    btnResult.setBackground(strExecStatus.equals("PASS")?Color.GREEN:Color.RED);
    btnResult.addActionListener(new ActionListener() {
		@Override
		public void actionPerformed(ActionEvent arg0) {
			if (Desktop.isDesktopSupported()) {
				File file= new File(strPathPDFReport);
				try {
					Desktop.getDesktop().open(file);
				}
				catch (IOException e) {
	...
	});	//end of btnResult.addActionListener
...
//	
//	public static ArrayList<String> listBatchTestReportsPath = new ArrayList<String>();
//	private JButton btnApplyConfigToAllRows;
//	private JButton btnApplyConfigToSelectedRows;
//	public static void main(String[] args) throws Exception {//Silent mode execution, invoked via command prompt
//		BatchRunner.Launch_SilentMode(args);
//	}
//	
//	private static void Launch_SilentMode(String[] args){//Private method since this is not likely to get called outside main()
//		if (args.length == 0){
...
//			ExecutorService executor = Executors.newFixedThreadPool(20); //count of parallel threads at a time
...
//		}
//		try {
//			//Batch runner could be opened as multiple instances by users, and from different places. Hence non static
//			BatchRunner batchRunner = new BatchRunner(listExclusions);				
//		} catch (Exception e) {
//			e.printStackTrace();
//		}	
...
//		txtConfigData.setTabSize(4);
//		txtConfigData.getDocument().putProperty("filterNewlines", Boolean.TRUE);
//		scrollPane = new JScrollPane(txtConfigData,ScrollPaneConstants.VERTICAL_SCROLLBAR_NEVER, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
//		panel.add(scrollPane,"3, 4, 9, 1, fill, center");
//		txtConfigData.setColumns(10);
//		
...
//		btnApplyConfigToSelectedRows.setToolTipText("Apply the config to all selected rows");
...
//		 	 	 
//
////Added by Abhi............RESULT COLOR CHANGER...........11/15/2016............................................./
//		  
...
//		        @Override
//		        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
//		            DefaultTableCellRenderer renderer1 = (DefaultTableCellRenderer) super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);			           
...
//		  	            if("Yes".equalsIgnoreCase(status)) renderer.setFont(new Font("", Font.BOLD, 12));         
...
//			    	txtCountParallelTests.setText("1");
//			    }				
//    			ExecutorService executor = Executors.newFixedThreadPool(nCountParallelTests); //count of parallel threads at a time
//    			for(int nRow: table.getSelectedRows()){
//    				
...
//    				
//    				final String strPackageAndClassName = model.getValueAt(nRow, 1).toString().trim();
//    				//final String[] arrayParams = new String[] { "Samsung S4", "Landscape"};
//    				final String[] arrayParams = new String[]{};
//    				executor.execute( new Runnable() { //Run asynchronously
//    				            @Override
...
//						if (endIndex != -1) sPackageName=strPackageAndClassName.substring(0, endIndex); // Extracting the packageName from the testcase   
...
//					    	if(iPackageCount!=hPackagesAndTestcases.size())sTestcaseNames=strPackageAndClassName+","; //checking the package change and setting testcase names 
//					    }
//					   
//					}
//				} catch (Exception e1) {
...
//	        	for(int nRow=0; nRow<nRowCount; nRow++){
//	        		
//	        		final String strOption = model.getValueAt(nRow, 0).toString().trim().toUpperCase();
//    				final String strPackageAndClassName = model.getValueAt(nRow, 1).toString().trim();	    					   	
//    			    final String[] arrayParams;
//    				
//    				//final String[] arrayParams = new String[] {"SEETEST","localhost","8889","ios_app:iPhone7","D:\\SeetestWorkspace"};	    				
//    				
...
//    			   }
//			
//    				if(strOption.trim().toUpperCase().equals("YES") || strOption.trim().toUpperCase().equals("Y")){
//    					model.setValueAt("Scheduled..", nRow, 2);
...
//				           
//				            	    for(int nRow=0; nRow<nRowCount; nRow++){
//				            	    	final String CurrPackageClassName = model.getValueAt(nRow, 1).toString().trim();
//				            	    	/*
...
//						if(endIndex != -1) sPackageName=line.toString().trim().substring(0, endIndex); // Extracting the packageName from the Testcase   
//						
//						if(bfirst){
...
//			model.addRow(new Object[]{"No",hPackagesAndTestcases.get(sSelectedPackage).split("\\,")[i],"not run",""}); 	
//		}
//		fnHighlightTheSavedRows();  
//	}
//			
